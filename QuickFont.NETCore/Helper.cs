...
namespace QuickFont
{
	...
	internal static class Helper
    {
		...
		/// <summary>
		/// Converts the given <see cref="Color"/> to a <see cref="OpenTK.Vector4"/>,
		/// such that XYZW = RGBA.
		/// <para></para>
		/// Vector values are normalized between 0.0 and 1.0
		/// </summary>
		/// <param name="color">The <see cref="Color"/> to convert</param>
		/// <returns>The <see cref="OpenTK.Vector4"/> of the color</returns>
        public static OpenTK.Vector4 ToVector4(OpenTK.Color color)
        {
            return new OpenTK.Vector4{X = (float)color.R / byte.MaxValue, Y = (float)color.G / byte.MaxValue, Z = (float)color.B / byte.MaxValue, W = (float)color.A / byte.MaxValue};
        }
    }
}
