...
namespace Oxy.Framework.AL
{
  ...
  public enum EfxEffectf
  {
    ...
    ///   noticeable with percussive sound sources. If you set a diffusion value of 0.0f, the later reverberation sounds like a
    ///   succession of distinct echoes. Range [0.0f .. 1.0f] Default: 1.0f
    /// </summary>
    ...
    ///   fed back for further echoes, damping results in an echo which progressively gets softer in tone as well as intensity.
    ///   Range [0.0f .. 0.99f] Default: 0.5f
    /// </summary>
    EchoDamping = 0x0003,
    ...
    ///   be panned hard left, and the second "tap" hard right. –1.0f gives the opposite result and values near to 0.0f result
    ///   in less emphasized panning. Range [-1.0f .. +1.0f] Default: -1.0f
    /// </summary>
    EchoSpread = 0x0005,
    ...
    ///   characteristic of rooms with highly reflective walls and/or small dimensions. Unit: Linear gain Range [0.0f .. 1.0f]
    ///   Default: 0.32f
    /// </summary>
  ...
  public enum EfxEffect3f
  {
    ...
    ///   vector controls the global direction of the reflections, while its magnitude controls how focused the reflections are
    ///   towards this direction. For legacy reasons this Vector3 follows a left-handed co-ordinate system! Note that OpenAL
    ///   uses a right-handed coordinate system. Unit: Vector3 of length 0f to 1f Default: {0.0f, 0.0f, 0.0f}
    /// </summary>
  ...
  public enum EfxEffecti
  {
    ...
    ///   vocal morpher is not necessarily intended for use on voice signals; it is primarily intended for pitched noise
    ///   effects, vocal-like wind effects, etc. Unit: Use enum EfxFormantFilterSettings Range [0 .. 29] Default: 10, "Phoneme
    ///   ER"
    /// </summary>
  ...
  public enum EfxAuxiliaryi
  {
    ...
    ///   the Auxiliary Effect Slot object will contain the effect type and have the same effect parameters that were stored in
    ///   the Effect object. Any Sources feeding the Auxiliary Effect Slot will immediate feed the new effect type and new
    ///   effect parameters.
    /// </summary>
